...
class Test_Obtain_Key_ID:
    ...
    def test_staging(self, key, public_key):
        ...
        satispaython.obtain_key_id(key, '623ECX')
    ...
    def test_production(self, key, public_key):
        ...
        satispaython.obtain_key_id(key, '623ECX', True)
        request = responses.calls[0].request
        assert len(responses.calls) == 1
        assert request.method == 'POST'
        assert json.loads(request.body) == {'public_key': public_key, 'token': '623ECX'}
...
class Test_Test_Authentication:

    @responses.activate
    @mark.freeze_time('Mon, 18 Mar 2019 15:10:24 +0000')
    def test_test_authentication(self, key_id, key):
        ...
        satispaython.test_authentication(key_id, key)
        request = responses.calls[0].request
        assert len(responses.calls) == 1
        assert request.method == 'POST'
...
class Test_Create_Paymet:
    ...
    def test_staging(self, key_id, key):
        ...
        satispaython.create_payment(key_id, key, 100, 'EUR', 'https://test.test?payment_id={uuid}', '2019-03-18T16:10:24.000Z', 'test_code', {'metadata': 'test'}, 'test_idempotency_key', True)
        request = responses.calls[0].request
        assert len(responses.calls) == 1
        assert request.method == 'POST'
    ...
    def test_production(self, key_id, key):
        ...
        satispaython.create_payment(key_id, key, 100, 'EUR', 'https://test.test?payment_id={uuid}', '2019-03-18T16:10:24.000Z', 'test_code', {'metadata': 'test'}, 'test_idempotency_key')
        request = responses.calls[0].request
        assert len(responses.calls) == 1
        assert request.method == 'POST'
        assert json.loads(request.body) == {
...
class Test_Get_Payment_Details:
    ...
    def test_staging(self, key_id, key, payment_id):
        ...
        satispaython.get_payment_details(key_id, key, payment_id, True)
        request = responses.calls[0].request
        assert len(responses.calls) == 1
    ...
    def test_production(self, key_id, key, payment_id):
        ...
        satispaython.get_payment_details(key_id, key, payment_id)
        request = responses.calls[0].request
        assert len(responses.calls) == 1
        assert request.method == 'GET'
        assert request.body is None
